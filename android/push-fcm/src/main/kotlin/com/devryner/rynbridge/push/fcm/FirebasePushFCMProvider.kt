...
class FirebasePushFCMProvider(
    private val onTokenRefresh: ((String, String, Map<String, com.devryner.rynbridge.core.BridgeValue>) -> Unit)? = null
) : PushFCMProvider {
    ...
    fun emitTokenRefresh(token: String) {
        onTokenRefresh?.invoke(
            "push-fcm",
            "tokenRefresh",
            mapOf("token" to com.devryner.rynbridge.core.BridgeValue.string(token))
        )
    }
}
